...
def main() -> int:
    ...
    parser.add_argument("dialog", type=Path); parser.add_argument("target", help="UUID de destino"); parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    try: report = incoming_jumps(load_json(args.dialog), args.target)
    except ValueError as error: print(f"Erro: {error}", file=sys.stderr); return 2
